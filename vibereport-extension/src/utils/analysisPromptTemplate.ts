...
function buildEvaluationReportSection(evaluationPath: string, projectVision?: ProjectVision): string {
  ...
  lines.push('');
  lines.push('#### (2) Feature-based Package Diagram');
  lines.push('');
  lines.push('Create a **Mermaid diagram** that shows the high-level package structure and feature modules in Korean.');
  lines.push('This diagram should illustrate how the main functionalities are organized within the codebase.');
  lines.push('');
  lines.push('Example format:');
  lines.push('');
  lines.push('```mermaid');
  lines.push('flowchart LR');
  lines.push('    subgraph Core["핵심 기능"]');
  lines.push('        Scanner["WorkspaceScanner<br/>워크스페이스 스캔"]');
  lines.push('        Snapshot["SnapshotService<br/>스냅샷 관리"]');
  lines.push('        Report["ReportService<br/>보고서 생성"]');
  lines.push('    end');
  lines.push('    subgraph UI["사용자 인터페이스"]');
  lines.push('        Commands["Commands<br/>VS Code 명령"]');
  lines.push('        Views["Views<br/>사이드바 뷰"]');
  lines.push('    end');
  lines.push('    Commands --> Scanner');
  lines.push('    Scanner --> Snapshot');
  lines.push('    Snapshot --> Report');
  lines.push('```');
  lines.push('');
  lines.push('Guidelines for the diagram:');
  lines.push('- Group related modules into logical subgraphs (e.g., Core, UI, Services, Utilities).');
  lines.push('- Show the main data/control flow between modules with arrows.');
  lines.push('- Keep the diagram focused on **feature organization**, not implementation details.');
  lines.push('- Label each node with both the module name and a brief Korean description.');
  lines.push('');
