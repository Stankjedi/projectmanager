...
- **버전 업데이트**: 문서/패키지 버전 0.4.39로 상승 및 VSIX 재생성
...
- **Redundant Activation Events 제거**: VS Code에서 자동으로 감지하는 redundant한 activation event들을 `package.json`에서 제거하여 IDE 경고를 해결하고 최적화했습니다.
...
- **버전 업데이트**: 로컬 보고서 동기화 및 릴리즈 패키지 생성
...
- **확장 활성화 누락으로 인한 뷰 무한 로딩/빈 화면 해결**: `onView:*` / `onCommand:*` activation event를 추가하여 사이드바 뷰/명령 실행 시 항상 확장이 활성화되도록 수정했습니다.
- **개발 환경 TypeScript 타입 오류 해결**: `tsconfig.json`의 `types`에 `vscode`를 포함하여 `import 'vscode'` 타입 해석이 정상 동작하도록 수정했습니다.
...
- **analysisRoot 경로 불일치 수정**: Summary/History 뷰 및 주요 명령들이 `analysisRoot` 하위에서 생성된 state/report 파일을 올바르게 읽고 열도록 수정했습니다.
- **Auto-update 루프 방지**: `analysisRoot` 사용 시에도 생성된 보고서 디렉토리/상태 파일 변경을 watcher가 무시하도록 처리했습니다.
- **설정 Export/Import 키 정합**: `analysisRoot`, `enableAutoUpdateReports`, `autoUpdateDebounceMs`를 내보내기/가져오기 대상에 포함하고, 레거시 키(`enableRealtimeAnalysis`, `realtimeDebounceMs`)는 자동 매핑합니다.
...
- **VSIX 의존성 누락으로 인한 확장 미작동 수정**: 런타임 의존성을 `out/extension.js`에 번들링하여 VSIX에 `node_modules` 없이도 확장이 정상 동작하도록 수정했습니다.

### 🛡️ 안정성

- **확장 활성화 안정화**: `jsonc-parser`를 동적 import로 전환해 의존성 로딩 문제로 확장 전체가 비활성화되지 않도록 처리했습니다.
- **워크스페이스 없는 창 대응**: 워크스페이스가 없는 상태에서 FileSystemWatcher 초기화로 활성화가 실패하지 않도록 가드 로직을 추가했습니다.
...
- **사이드바 무한 로딩 수정**: `SummaryViewProvider`에서 비동기 상태 로딩 시 발생하는 예외를 catch하여 무한 로딩 방지. 오류 발생 시 "아직 보고서가 생성되지 않았습니다" 메시지 표시.
- **디버그 로깅 추가**: 워크스페이스 경로 없음 및 상태 로딩 실패 시 Output 채널에 로그 기록.
