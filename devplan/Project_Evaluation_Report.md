...
> 아래 점수는 v0.3.8 기준 **10차 평가 결과**입니다.  
> v0.3.8에서 OPT 항목 영어 출력, 다중 선택 지원, OPT 상태 기반 정렬 기능이 추가되었습니다.
...
- **코드 구조**는 v0.3.5 이후 불필요한 코드가 제거되어 더욱 간결해졌습니다. v0.3.8에서는 OPT 파싱 로직이 Prompt.md 기반으로 개선되었습니다.
- **테스트·CI**는 86개 단위 테스트와 GitHub Actions 워크플로우로 기본 방어선이 구축되어 있습니다.
- **문서화·UX**는 README 업데이트와 사이드바 UI가 직관적이며, CHANGELOG 섹션이 추가되었습니다.
